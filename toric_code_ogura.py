...
class ToricCode:
    ...
    def errors_reshape(self, errors) -> np.array:
        errors = errors
    ...
    def generate_syndrome_Z(self, errors) -> np.array:  # 面
        ...
        while i < self.size:  # 行
            ...
            while j < self.size:  # 列
                ...
                # Z周りの４つのqubitを確認
                syndZ[i,j] = 0.5 * (1-np.cos(np.pi * count_Z_operator))
    ...
    def generate_syndrome_Z_differentiable(self, errors) -> np.array:
        
        syndZ = np.zeros((self.size, self.size),dtype = np.float32)
        errors = errors[0,0,:,:]
        i,j = 0, 0
        while i < self.size:
            j = 0
            while j < self.size:
                ...
                if j == self.size -1: #migi
                    count_Z_operator += errors[2 * i + 1][0]
                else:
                    count_Z_operator += errors[2 * i + 1][j + 1]
                if i == self.size -1: #sita
                    count_Z_operator += errors[0][j]
                else:
                    count_Z_operator += errors[2 * i + 2][j]
                
                count_Z_operator += errors[2 * i + 1][j] #hidari
                count_Z_operator += errors[2 * i][j] #ue
        ...
        while i < self.size:
            ...
            while j < self.size:
                ...
                if errors[2 * i][j] == 2 or errors[2 * i][j] == 3:  # 右
                    count_X_operator += 1
                # X周りの４つのqubitを確認
                syndX[i,j] = 0.5 * (1-np.cos(np.pi * count_X_operator))#微分可能
    ...
    def generate_syndrome_X_differentiable(self, errors) -> np.array: #tenn
        syndX = np.zeros((self.size, self.size), dtype = np.float32)
        errors = errors[0,0,:,:]
        i, j = 0, 0
        while i < self.size:
            j = 0
            while j < self.size:
                ...
                if j == 0:
                    count_X_operator += errors[2 * i][self.size - 1]
                else:
                    count_X_operator += errors[2 * i][j - 1]
                if i == 0:
                    count_X_operator += errors[2 * self.size - 1][j]
                else:
                    count_X_operator += errors[2 * i - 1][j]
                
                count_X_operator += errors[2 * i + 1][j]
                count_X_operator += errors[2 * i][j]
...
errors_onehot = F.one_hot(errors_tensor, num_classes=4)
re_errors = torch.argmax(errors_onehot, dim=2)
...
print("reshaped:", errors_reshape_X)
print("reshaped:", errors_reshape_Z)
